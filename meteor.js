...
var basePath = './';
var buildPath = path.resolve(argPath);
...
module.exports = {
  ...
  addIndexFile: function(program, callback) {
    ...
    var content = fs.readFileSync(program.template || path.resolve(__dirname, 'index.html'), {
      encoding: 'utf-8',
    });
    var head;
    try {
      head = fs.readFileSync(path.join(buildPath, 'head.html'), {
        encoding: 'utf8',
      });
    } catch (e) {
      head = '';
